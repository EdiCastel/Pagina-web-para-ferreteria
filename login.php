...
if(isset($_POST['form_email']) && isset($_POST['form_password'])){
	...
	if(!empty(trim($_POST['form_email'])) && !empty(trim($_POST['form_password']))){
		...
		$query = mysqli_query($db_connection, "SELECT * FROM `usuarios` WHERE email = '$form_email' or usuario = '$form_email'");
		...
		if(mysqli_num_rows($query) > 0){
				...
				if($form_email == 'Admin@gmail.com' && $verifico_password == '1234' ){
