...
if(isset($_POST['form_usuario']) && isset($_POST['form_email']) && isset($_POST['form_password']) && isset($_POST['nombre']) && isset($_POST['apellidos']) && isset($_POST['telefono'])){

	// Verifico que los campos enviados desde el formulario no esten vacios y quito espacios en blanco
	if(!empty(trim($_POST['form_usuario'])) && !empty(trim($_POST['form_email'])) && !empty($_POST['form_password']) && !empty($_POST['nombre']) && !empty($_POST['apellidos']) && !empty($_POST['telefono'])){
		...
		$apellidos = mysqli_real_escape_string($db_connection, htmlspecialchars($_POST['apellidos']));
		$telefono = mysqli_real_escape_string($db_connection, htmlspecialchars($_POST['telefono']));
		...
		$verifico_email = mysqli_query($db_connection, "SELECT `email` FROM `usuarios` WHERE email = '$form_email'");
			...
			if(mysqli_num_rows($verifico_email) > 0){    
				...
				$inserto_usuario = mysqli_query($db_connection, "INSERT INTO `usuarios` (id, usuario, email, password, nombre,apellidos, telefono) VALUES (NULL, '$form_usuario', '$form_email', '$usuario_hash_password','$nombre','$apellidos','$telefono')");
